...
回到我们在 2.2 节中运行的 [2-train_dpo.py](../../code/chapter02_dpo/2-train_dpo.py)，代码里加载的 `preference_data.json` 正是这种格式。数据加载部分将 JSON 文件解析为 `prompt`、`chosen`、`rejected` 三个字段，分别对应符号表中的 $x$、$y_w$、$y_l$：
...
回到 [2-train_dpo.py](../../code/chapter02_dpo/2-train_dpo.py) 的代码，`DPOTrainer` 在初始化时只接收一个 `model` 参数：
