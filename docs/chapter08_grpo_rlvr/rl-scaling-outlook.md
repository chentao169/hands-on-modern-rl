...
RLMT 站在第 8 章 RLVR 和第 10 章 RLHF 的交叉点上：

| 概念来源                | 在 RLMT 中的角色             |
| ----------------------- | ---------------------------- |
| RLVR 的长思考链（Ch8）  | 保留"先思考再回答"的输出结构 |
| RLHF 的偏好奖励（Ch10） | 用偏好 RM 替代规则验证       |
...
到这里，我们已经覆盖了 RL 训练的三个核心维度。但还有一种后训练方法没有提到——它不需要 RM，不需要标准答案，只需要一个现成的强模型就能工作。那就是**知识蒸馏**，具体来说就是**在线策略蒸馏（OPD）**。Qwen3、GLM-5、MiMo-V2-Flash 都在用。我们下一节就来拆解它。
