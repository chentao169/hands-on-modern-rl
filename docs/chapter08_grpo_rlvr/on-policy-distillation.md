...
回顾整个第 8 章，我们已经覆盖了 LLM 后训练的四条核心路线：

| 路线 | 核心思想 | 何时选择 |
| --- | --- | --- |
| GRPO (8.1-8.2) | 组内归一化省掉 Critic | 有可验证奖励的场景 |
| DAPO / RLVR (8.3) | 动态采样 + 规则奖励 | 大规模 RL 训练 |
| RL Scaling (8.4) | 更多算力 → 更强推理 | 追求极致性能 |
| **知识蒸馏 / OPD (本节)** | **用教师模型当密集奖励** | **有强模型可用，想快速迁移能力** |
...
从第 5 章的策略梯度定理，到第 6 章的 PPO 裁剪机制，到第 7 章的 DPO 数学推导，再到第 8 章的 GRPO、RLVR 和知识蒸馏——你已经掌握了现代 RL 训练从理论到实践的核心知识。接下来的篇章将带你进入更前沿的领域：连续控制、RLHF 全流程、Agentic RL、以及更多令人兴奋的方向。
