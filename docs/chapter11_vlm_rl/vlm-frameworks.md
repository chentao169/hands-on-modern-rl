...
把本章的 VLM RL 和[第 10 章](../chapter12_agentic_rl/intro)的 Agent RL 结合，会面临三个额外的挑战：
...
**3. 跨模态 Credit Assignment。** 在一条 10 轮的轨迹中，第 2 轮的视觉理解错误可能导致第 5 轮的工具调用失败。这比纯文本 Agent 的 credit assignment 更难，因为跨模态的错误传播链更长、更隐晦。[第 10 章](../chapter12_agentic_rl/multi-turn-rl)讨论的 ORM vs PRM 取舍在这里更加突出。
...
2. **再练工具调用**：用[第 10 章的工具调用 RL](../chapter12_agentic_rl/tool-use-agents)训练基本的工具使用模式
